...
import { getHills } from './get-hills';
...
export const Hills: React.FC<HillsProps> = ({
	...
	const stepSize = (w - 2 * padWidth) / (amplitudes.length - 1);
	const lines = Array.from({ length: copies }).map((_, lineIndex) => {
		const shift = 3 * lineIndex;
		const lineValues = rotate(amplitudes, shift);
		return lineValues.map((v, i) => ({
			...
			y:
				...
				h *
				v *
				(1.2 - 0.5 * random(lineIndex * amplitudes.length + i)),
		}));
	});
